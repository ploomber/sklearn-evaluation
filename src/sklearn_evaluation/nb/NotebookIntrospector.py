...
def _process_output(output):
    ...
    if out_type == "stream":
        ...
        raise NotImplementedError(
            'Processing output of type "{}" is not ' "implemented".format(out_type)
        )
...
def _parse_output(output, literal_eval, to_df, text_only):
    if not text_only and "image/png" in output:
        ...
        return out if not literal_eval else _safe_literal_eval(out, to_df=to_df)
...
class NotebookIntrospector(Mapping):
    ...
    def __init__(self, path, literal_eval=True, to_df=False):
        ...
        self.tag2output = {
            k: _parse_output(v, literal_eval=literal_eval, to_df=to_df, text_only=False)
    ...
    def _tag2output(self):
        ...
        cells = [_process_cell(c) for c in self.nb.cells if c.cell_type == "code"]
        ...
        # the last valid one
        cells = {
            k: _filter_and_process_outputs(outputs) for k, outputs in cells.items()
        }
    ...
    def to_json_serializable(self):
        return {
            k: _parse_output(
                v, literal_eval=self.literal_eval, to_df=False, text_only=True
            )
            for k, v in self.tag2output_raw.items()
        }
