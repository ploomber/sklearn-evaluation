...
def test_1d_incorrect_features():
    with pytest.raises(ValueError) as e:
        ...
        Rank1D(features=['Sepal length', 'Sepal width'],
               algorithm='shapiro').feature_ranks(X)
...
def test_1d_numpy():
    ...
    Rank1D(algorithm='SHAPIRO', orient='h',
           figsize=(13, 7)).feature_ranks(X_numpy)
...
def test_1d_custom_ranks():
    ...
    Rank1D(features=['Feature1', 'Feature2', 'Feature3'],
           orient='h',
           figsize=(13, 7)).feature_ranks_custom_algorithm(ranks)
...
def test_1d_custom_ranks_incorrect_dimension():
    ...
    with pytest.raises(ValueError) as e:
        Rank1D(features=['Feature1',
                         'Feature2'], ).feature_ranks_custom_algorithm(ranks)
...
def test_2d_pearson_colormap():
    Rank2D(algorithm='pearson',
           features=features,
           figsize=(12, 12),
           colormap='PuBuGn').feature_ranks(X)
...
def test_2d_incorrect_features():
    with pytest.raises(ValueError) as e:
        ...
        Rank2D(features=['Sepal length', 'Sepal width'],
               algorithm='pearson').feature_ranks(X)
        assert "number of supplied feature names does not match the number " \
               "of columns in the training data." in str(e.value)
...
def test_2d_custom_ranks():
    ...
    Rank2D(features=['Feature1', 'Feature2', 'Feature3', 'Feature4'],
           figsize=(9, 12)).feature_ranks_custom_algorithm(ranks)
