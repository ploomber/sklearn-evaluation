...
class TestPlotSilhouette(TestCase):
    ...
    def test_string_classes(self):
        ...
        plot.silhouette_plot_from_results(self.X, convert_labels_into_string(cluster_labels))

    def test_cmap(self):
        np.random.seed(0)
        clf = KMeans()
        cluster_labels = clf.fit_predict(self.X)
        plot.silhouette_plot_from_results(self.X, cluster_labels, cmap='Spectral')
        plot.silhouette_plot_from_results(self.X, cluster_labels, cmap=plt.cm.Spectral)
    ...
    def test_ax(self):
        ...
        out_ax = plot.silhouette_plot_from_results(self.X, cluster_labels, ax=ax)
        assert ax is out_ax

    def test_array_like(self):
        plot.silhouette_plot_from_results(self.X.tolist(), self.y.tolist())
        plot.silhouette_plot_from_results(self.X.tolist(), convert_labels_into_string(self.y))